/*
  ...
  Corrected version: uses supabaseClient to avoid colliding with the
  Supabase CDN's global `supabase` variable.
...
authForm.onsubmit = async event => {
  ...
  /*
    ...
    The user's chosen username is what is displayed publicly.
  */
  ...
  if (mode === "register") {
    ...
    if (!data.user) {
      errorText("Account could not be created.");
      return;
    }

    /*
      Email confirmation was disabled in the Supabase setup.
      Therefore the user can log in immediately.
    */
  ...
  currentProfile = sessionData.session
    ? await loadProfile(sessionData.session.user.id)
    : null;

  if (!currentProfile) {
    errorText(
      "Login succeeded but the profile was not found. Please contact the site administrator."
    );
    return;
  }
...
async function start() {
  ...
  if (data.session) {
    currentProfile = await loadProfile(data.session.user.id);
  }
  ...
  supabaseClient
    ...
    .subscribe(status => {
      if (status === "CHANNEL_ERROR") {
        console.error("Could not establish realtime chat connection.");
      }
    });
  ...
  supabaseClient
    ...
    .subscribe(status => {
      if (status === "CHANNEL_ERROR") {
        console.error("Could not establish realtime status connection.");
      }
    });
